...
function draw() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let col = obj.col;
    let isHover = (dist(mouseX, mouseY, obj.x, obj.y) < obj.radius);
    if (isHover) {
      col = color(255, 0, 0);
    }

    if (obj.y - obj.radius > height) {
      obj.radius = random(20, 60);
      obj.x = random(width);
      obj.y = -obj.radius;
    ...
    if (obj.snowType === 1) {
      drawSnow01(obj.x, obj.y, obj.angle, obj.radius, col);
    }
    if (obj.snowType === 2) {
      drawSnow02(obj.x, obj.y, obj.angle, obj.radius, col);
    }
    if (obj.snowType === 3) {
      drawSnow03(obj.x, obj.y, obj.angle, obj.radius, col);
    }
    if (obj.snowType === 4) {
      drawSnow04(obj.x, obj.y, obj.angle, obj.radius, col);
    }
    if (obj.snowType === 5) {
      drawSnow05(obj.x, obj.y, obj.angle, obj.radius, col);
    }
  }
